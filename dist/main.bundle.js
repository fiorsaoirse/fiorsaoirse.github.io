...
var renderResponse = function (response) {
    ...
    if (!incomplete_results) {
        ...
        items.map(function (currentElement) {
...
var getRepos = function () {
    return fetch("https://api.github.com/search/repositories?q=" + (input.value).replace(' ', '+') + "+in:name")
        .then(function (res) { return res.json(); }, function (err) { return console.error(err); });
};
var sendRequestToGit$ = getEvent$.pipe(operators_1.filter(function (event) { return event.code === 'Enter'; }), operators_1.mergeMap(function (_keypress) { return getRepos(); }));
sendRequestToGit$.subscribe(function (result) { return renderResponse(result); });
